  ...
  const getPrimaryImage = () => {
    if (product?.images && product.images.length > 0) {
      ...
      if (primaryImage.image_path) {
        return `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5500'}${primaryImage.image_path}`;
      }
    }
    // Return a data URI for a simple placeholder (gray square with text)
          ...
          <div>
            <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4">
              <img
                ...
                onError={(e) => {
                  // Fallback to a simple gray background if image fails
                  const target = e.target as HTMLImageElement;
                  target.style.display = 'none';
                  const parent = target.parentElement;
                  if (parent) {
                    parent.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400 bg-gray-100"><span>No Image</span></div>';
                  }
                }}
              />
            </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  {uniqueColors.map((color) => (
                    <button
                      key={color?.color_id}
                      onClick={() => setSelectedColor(color?.color_id || null)}
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  {uniqueSizes.map((size) => (
                    <button
                      key={size?.size_id}
                      onClick={() => setSelectedSize(size?.size_id || null)}
                      className={`px-4 py-2 border-2 rounded ${
